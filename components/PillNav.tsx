  ...
  useEffect(() => {
    const layout = () => {
      circleRefs.current.forEach((circle, index) => {
        ...
        if (white) gsap.set(white, { y: shiftDist, opacity: 0 });
        ...
        if (label) {
          tl.to(label, { y: -shiftDist, opacity: 0, duration: 0.35, ease, overwrite: 'auto' }, 0);
        }

        if (white) {
          tl.fromTo(white, { y: shiftDist, opacity: 0 }, { y: 0, opacity: 1, duration: 0.35, ease, overwrite: 'auto' }, 0);
        }
        ...
        {logo &&
          (isRouterLink(firstHref) ? (
            ...
            <a
              ...
              aria-label="Home"
              onMouseEnter={handleLogoEnter}
              onClick={e => handleNavClick(e, firstHref)}
              ref={el => {
                logoRef.current = el;
              }}
            >
